...
namespace hands_viewer.cs
{
    class HandsRecognition
    {
        ...
        float GetXPosition(float cursorXPos)
        {
            ...
            if (cursorXPos >= _info.width - CURSOR_FACTOR_X)
            {
                return (float)(_info.width - 1.0);
            }
            return ((cursorXPos - CURSOR_FACTOR_X) / (_info.width - 2 * CURSOR_FACTOR_X)) * _info.width;
        }
        ...
        float GetYPosition(float cursorYPos)
        {
            ...
            if (cursorYPos >= _info.height - CURSOR_FACTOR_Y_DOWN)
            {
                return (float)(_info.height - 1.0);
            }
            return ((cursorYPos - CURSOR_FACTOR_Y_UP) / (_info.height - CURSOR_FACTOR_Y_UP - CURSOR_FACTOR_Y_DOWN)) * _info.height;
        }
            ...
            if (_form.GetJointsState() || _form.GetSkeletonState()  || _form.GetExtremitiesState())
            {
                ...
                if (numOfHands > 0)
                {
                    ...
                    if (_form.GetCursorModeState())
                        _form.DisplayCursor(numOfHands, _mCursorPoints, _mAdaptivePoints, _mCursorClick);
